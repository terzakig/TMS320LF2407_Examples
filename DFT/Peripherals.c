...
void initADC(void) {
  ...
  ADCTRL1bits->bit.ACQPS = 15;   // Acquisition window = 32*Tclk=32*Clkout 
  								// (16 x sampling time = 0.172 ms conversion time)
  ...
  *MAXCONV = 15; // 16 conversions by the cascaded sequencer 
  ...
  CHSELSEQ1bits->all = 0x1111;
  CHSELSEQ2bits->all = 0x1111;
  CHSELSEQ3bits->all = 0x1111;
  CHSELSEQ4bits->all = 0x1111;
  
  // Sampling Channel 1 (16 times - oversampling) 
...
void initTimer1(void) {
	 ...
	 GPTCONBbits->bit.T3PIN = 0;
	 GPTCONBbits->bit.T3TOADC = 2; // start on period
	 GPTCONBbits->bit.T4PIN = 0;
	 GPTCONBbits->bit.T4TOADC = 0;
	 GPTCONBbits->bit.TCOMPOE = 0;

	 // Timer 1 Initialization
     *T1CNT = 0;
	 *T1PR = 750;   // 750/40x10^6 = 18.75 usec timer period
	 *T1CMPR = 500; // 25 usec signal period (pulse length = 18.75 usec ) 
