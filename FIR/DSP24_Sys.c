...
void initSystem(void) {
    ...
    *SCSR1 = 0x0FE;
	...
	bit 4         1:      0 = enable CAN module clock
	bit 3         1:      0 = enable EVB module clock
	...
	*INTMASK = 0;   // mask core interrupts
}

void initIO(void) {
 
  // Timer 1
  MCRAbits->bit.T1PWMT1CMP_GPIOB4 = 1;  // T1 Compare pin enabled 
  
  // SPI
  // setting up SPI pins
  MCRBbits->bit.SPISIMO_GPIOC2 = 1; // Use the pin as SPISIMO
  MCRBbits->bit.SPISOMI_GPIOC3 = 1; // Use the pin as SPISOMI
  MCRBbits->bit.SPISTE_GPIOC5 = 1;  // Use the pin as SPISTE
  MCRBbits->bit.SPICLK_GPIOC4 = 1;  // Use the pin as SPICLK
